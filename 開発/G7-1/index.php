<!DOCTYPE html>
<html lang="ja">
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <title>管理者</title>
    <link rel="stylesheet" href="../Sample/template/vendors/typicons.font/font/typicons.css">
    <link rel="stylesheet" href="../Sample/template/vendors/css/vendor.bundle.base.css">
    <link rel="stylesheet" href="../Sample/template/css/vertical-layout-light/style.css">
    <link rel="shortcut icon" href="../Sample/template/images/favicon.png" />
    <link rel="stylesheet" href="css/style.css">
  </head>
	<body>
		<h1>管理者画面</h1>
		<!--location.hrefでページ遷移(html用記述)-->
    <button type="button" class="btn btn-social-icon-text btn-linkedin" onclick="location.href='../G7-2/index.php'"><i class="typcn typcn-social-linkedin-circular"></i>在庫画面へ</button>
    <img src="../image/AsoCityロゴ2.png" class="rogo">
  	</body>
</html>